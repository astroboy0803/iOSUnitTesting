...
//        confirmPasswordTextField.resignFirstResponder()
    ...
    private func showAlert(message: String, okAction: @escaping (UIAlertAction) -> Void) {
        ...
        let okButton: UIAlertAction = .init(title: "OK", style: .default, handler: okAction)
    ...
    private func validateInputs() -> Bool {
        ...
        if newPasswordTextField.text?.count ?? 0 < 6 {
            showAlert(message: "The new password should at least 6 characters.") { [weak self] _ in
        ...
        if newPasswordTextField.text != confirmPasswordTextField.text {
            showAlert(message: "Ther new password and the confirmation password don't match. Please try again.") { [weak self] _ in
                self?.resetNewPasswords()
            }
            return false
        }
